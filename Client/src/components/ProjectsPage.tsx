...
export function ProjectsPage() {
  ...
  // 🧠 New: Comment states (per project)
  // const [comments, setComments] = useState({}); 
  const [comments, setComments] = useState(
  () =>
    projects.reduce((acc, p) => {
      const randomCount = Math.floor(Math.random() * 151) + 50; // 50–200 range
      acc[p.id] = randomCount;
      return acc;
    }, {})
);
  const [showCommentBox, setShowCommentBox] = useState({}); // { [projectId]: boolean }
  const [newComment, setNewComment] = useState({}); // { [projectId]: string }

  // 🔄 Toggle comment box for a specific project
  const handleCommentClick = (projectId) => {
